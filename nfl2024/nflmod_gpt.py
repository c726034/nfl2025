...
with open('emails.csv', newline='') as e:
    reader = csv.reader(e)
...
def get_scores(day1=None, day2=None):
    ...
    scoresdf['nflweek'] = (scoresdf.date-dt.timedelta(days=3)).dt.isocalendar().week + ((scoresdf.date.dt.year-2024)*52) - 35
    ...
    contestbeta = gc.open("NFL_Pool_2024")
...
def get_picks(picksday='X', w=None):
    '''
    Retrieve picks from Form data, process picks, print late picks (if any), and write picks to the contest spreadsheet.

    Arguments:
    - picksday: Specify which picks to write to the spreadsheet. Options are:
        'T' - Thursday picks only
        'A' - Saturday picks only
        'W' - Wednesday picks only
        'S' - All picks for the week (includes Thursday, Saturday, Sunday/Monday, etc.)
        Any other value will process picks without writing to the spreadsheet.

    - w: NFL week to process. Defaults to the current NFL week if not provided.

    Functionality:
    - Pulls picks for the specified week from the response sheet.
    - Identifies and processes picks made for early games (Thursday, Saturday, and Wednesday).
    - Filters and formats the selected picks based on 'picksday'.
    - Writes picks to the contest spreadsheet in a specified range.
    - Prints a list of late picks if any are detected.
    '''
    try:
        w = int(w)
    except ValueError:
    ...
    sides = pd.read_csv('sides.csv')
    names = pd.read_csv('names.csv')

    gc = gspread.service_account(filename="service_account.json")
    contest = gc.open("NFL_Pool_2024")
    picksheet = gc.open("NFL 2024 (Responses)")
    gspicks = picksheet.worksheet("Form Responses 1")

    # PROCESS PICKS TO WIDE FORMAT
    ...
    for p in ['pick1', 'pick2', 'pick3', 'pick4', 'pick5']:
        weekpicks[p] = weekpicks['picks'].str.split("\),", expand=True)[int(p[-1]) - 1]
        if p in weekpicks:
    ...
    # MERGE PICKS TO SINGLE COLUMN
    picks = pd.DataFrame()
    for p in ['pick1', 'pick2', 'pick3', 'pick4', 'pick5']:
        if p in weekpicks.columns:
            pickx = weekpicks[['timestamp', 'name', p]].rename(columns={p: 'pick'})
            picks = pd.concat([picks, pickx], ignore_index=True)
    picks = picks.dropna()

    if picks.empty:
        print("No picks found for the specified week.")
        return None
    ...
    picks['thurs'] = np.where(picks.weekday == 3, 1, 0)
    picks['fri'] = np.where(picks.weekday == 4, 1, 0)
    picks['sat'] = np.where(picks.weekday == 5, 1, 0)
    picks['wed'] = np.where(picks.weekday == 2, 1, 0)

    # Ensure rank column is created
    picks['rank'] = picks.sort_values(
        by=['thurs', 'fri', 'sat', 'timestamp'],
        ascending=[False, False, False, False]
    ).groupby('name').cumcount(ascending=True) + 1

    # Predefine Outputs for Writing
    def prepare_output(day_filter):
        picksout = picks[day_filter]
        picksout = names.merge(picksout, how='left', on='name')
        picksout = picksout.pivot(values='pick', index='rank', columns=['name']).reset_index()
        return picksout.drop(columns=['rank']).dropna(how='all').fillna('')

    thpicksout = prepare_output(picks.thurs == 1)
    satpicksout = prepare_output(picks.sat == 1)
    wedpicksout = prepare_output(picks.wed == 1)
    picksout = prepare_output(picks['rank'] <= 5)
    ...
    if picksday == 'A':
        pickinput.update(f'{startcell}:{endcell}', satpicksout.values.tolist())
    if picksday == 'W':
        pickinput.update(f'{startcell}:{endcell}', wedpicksout.values.tolist())
    if picksday == 'S':
        pickinput.update(f'{startcell}:{endcell}', picksout.values.tolist())

    # LATE PICKS LOGIC
    picks['timestamp'] = pd.to_datetime(picks['timestamp']).dt.tz_localize('US/Mountain')
...
def get_lines(write = 'N', send = 'N', recip = emails, nflweek=None):
    ...
    '''
    if nflweek == None:
        nflweek = int((dt.datetime.today()-dt.timedelta(2)).strftime('%U'))-34
    w = nflweek
    ...
    def email_lines(recip):
        ...
        <body>
            ...
            <p><a href="https://t.ly/WcIkt">Make your picks here.</a></p>
            <p></p> <!-- Blank line -->
            <p><a href="https://t.ly/OlLCw">See the standings here!</a></p>
        </body>
        </html>
        """
        ...
        msg["From"] = f"Degen Contest 2024 <degens2019@gmail.com>"
        msg["To"] = recip
